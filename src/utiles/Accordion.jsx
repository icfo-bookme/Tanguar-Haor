...
const Accordion = ({ facilities = { facilities: [] } }) => {
  ...
  const toggleAccordion = (facilityType, event) => {
          ...
          {["Summary", "Description"].map((tab) => (
            <div
              ...
              className={`bg-white flex font-bold md:mx-0 items-center flex-shrink-0 cursor-pointer py-2 border-b-4 ${
                activeTab === tab
                  ? "border-blue-500 border-b-2 bg-white text-[#00026E] md:mr-5"
          ...
          {staticFacilityTypes.map((facilityType, index) => {
            ...
            return (
              <div key={index} className="w-full mt-[10px] cursor-pointer">
                {/* Accordion Header */}
                <div
                  className="cursor-pointer p-3 rounded-md shadow-sm hover:bg-gray-300 flex items-center justify-between bg-white z-10"
                  onClick={(event) => toggleAccordion(facilityType, event)}
                >
                ...
                <div
                  className={`transition-all duration-300 ease-in-out overflow-hidden ${
                    isOpen ? "opacity-100 p-4 block mt-[10px] max-h-screen" : "opacity-0 p-0 hidden max-h-0"
                  }`}
                >
